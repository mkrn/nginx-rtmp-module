    ...
    ngx_int_t boundary, ngx_uint_t flush_rate)
{
    ...
    if (ctx->opened) {
        ...
        ngx_log_error(NGX_LOG_DEBUG, s->connection->log, 0,
            "hls: '%s' update fragment, ts: %ld frag_ts: %ld, boundary: %i, flushrate: %lu", ctx->stream.data, (long)ts, (long)ctx->frag_ts, boundary, flush_rate);
    ...
    if (boundary || force) {
        ngx_log_error(NGX_LOG_ERR, s->connection->log, 0,
            "hls: splitting: ts: %ld frag_ts: %ld, duration: %.3f", (long)ts, (long)ctx->frag_ts, (long)f->duration);
        ngx_rtmp_hls_close_fragment(s);
    ...
    ngx_chain_t *in)
{
    ...
    int64_t d = (int64_t) (frame.dts - ctx->frag_ts);

    // Video Frame too out of sync, dont write it at all
    if (d < 0) {
        ngx_log_error(NGX_LOG_DEBUG, s->connection->log, 0, "Dropping out of sync video");
        return NGX_OK;
    }
